    ...
    public class IssuesController : ListController<IssueModel, IssuesFilterModel>
    {
        ...
        protected override List<IssueModel> FilterModel(List<IssueModel> model, IssuesFilterModel filter)
        {
            ...
            if (order == IssuesFilterModel.Order.Local_Id)
                return model.OrderBy(x => x.LocalId).ToList();
            else
        ...
        {
            Model.Data.RemoveAll(a => a.LocalId == issue.LocalId);
            Render();
        }
        ...
        private IssuesModel GetData(int start = 0, int limit = 50, IEnumerable<Tuple<string, string>> additionalFilters = null)
        {
